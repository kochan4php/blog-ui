...
const UIDesign = () => {
  ...
  const { uiuxdesign } = router.query;
  const [dataPosts, setDataPosts] = useState([]);

  useEffect(() => {
    const postsByCategory = blog.filter((item) => item.category === uiuxdesign);
  ...
  return (
    <section className="mb-6">
      <div className="text-center mb-10">
        <h1 className="text-slate-300 text-4xl font-semibold">UI UX Design</h1>
      </div>
